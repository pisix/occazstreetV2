...
export class MessagesPage extends AbstractLoggedPage implements OnInit, OnDestroy,AfterViewChecked {
  ...
  constructor(zone: NgZone, protected navCtrl: NavController, protected navParams: NavParams, public chatService: ChatService, element: ElementRef, public messageService: MessageService, public el: ElementRef) {
    super();
    debugger;
    this.initData();
  }
  ...
  initData(): void {
    if (localStorage.getItem(GlobalsConstants.USER_LOGGED)) {
      this.selectedChat = this.navParams.get('chat');
      this.article = this.navParams.get('startChat');
      ...
      if (this.navParams.get('startChat')) {
        this.title = this.navParams.get('startChat').title;
        this.prix = this.navParams.get('startChat').prix;
        this.article = <Article>this.navParams.get('startChat');
        this.loggedUser = JSON.parse(localStorage.getItem(GlobalsConstants.USER_LOGGED));
        ...
        conversation.utilisateur2 = this.navParams.get('startChat').utilisateur.id;
